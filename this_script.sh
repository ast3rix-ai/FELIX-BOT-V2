...
# base64 decode helper (macOS uses -D, GNU uses -d)
b64_decode_file() {
  local in="$1" out="$2"
...
mapfile -t RAW_URLS < <(jq -r '.files[] | .raw_url' "$GIST_JSON")
mapfile -t FILENAMES < <(jq -r '.files[] | .filename' "$GIST_JSON")
...
for i in "${!RAW_URLS[@]}"; do
  ...
  if [[ "$name" =~ \.zip$ ]]; then
    ...
    b64_decode_file "$path" "$ZIP_PATH" || { echo "ERROR: base64 decode failed for $name"; exit 1; }
    FOUND_ARCHIVE=1
  else
